...
// Interface yang akan diimplementasikan
interface PelaporanKeuangan {
    // Method untuk menghitung total pendapatan
    double hitungTotalPendapatan();
    
    // Method untuk membuat laporan transaksi
    String buatLaporanTransaksi(String periode);
}
...
class LoginAdmin {
    ...
    private void tambahAdmin() {
        ...
        System.out.println("Berhasil menambahkan admin");
    }
...
// Kelas yang mengimplementasikan interface PelaporanKeuangan
class LaporanKeuangan implements PelaporanKeuangan {
    private final List<Layanan> daftarLayanan;
    
    public LaporanKeuangan(List<Layanan> daftarLayanan) {
    ...
    public double hitungTotalPendapatan() {
        ...
        for (Layanan layanan : daftarLayanan) {
            if (layanan.getStatus().equalsIgnoreCase("Selesai")) {
                total += layanan.getHargaLayanan();
            }
        }
        return total;
    }
    ...
    @Override
    public String buatLaporanTransaksi(String periode) {
        if (daftarLayanan.isEmpty()) {
            return "Tidak ada transaksi untuk periode " + periode;
        }

        StringBuilder laporan = new StringBuilder();
        laporan.append("=== LAPORAN KEUANGAN PERIODE ").append(periode).append(" ===\n\n");
        laporan.append(String.format("| %-5s | %-15s | %-12s | %-20s | %-10s |\n", 
                "No", "Tanggal", "Status", "Jenis Cucian", "Harga"));
        laporan.append("-".repeat(77)).append("\n");

        boolean adaTransaksi = false;
        double total = 0;

        for (Layanan layanan : daftarLayanan) {
            String tanggal = layanan.getTanggalTransaksi(); // format: "Wed May 01 12:34:56 WIB 2025"
            String[] parts = tanggal.split(" ");
            if (parts.length >= 6) {
                String bulan = parts[1];
                String tahun = parts[5];
                String bulanTahun = bulan + " " + tahun;

                if (bulanTahun.equalsIgnoreCase(periode)) {
                    adaTransaksi = true;
                    laporan.append(String.format("| %-5d | %-15s | %-12s | %-20s | Rp%-8d |\n", 
                            layanan.getNoPesanan(), 
                            tanggal.substring(0, 10), 
                            layanan.getStatus(),
                            layanan.jenisCucian.jenisCucian,
                            layanan.getHargaLayanan()));
                    total += layanan.getHargaLayanan();
                }
            }
        }
        ...
        laporan.append("-".repeat(77)).append("\n");
        laporan.append(String.format("| %-61s | Rp%-8.0f |\n", "Total Pendapatan", total));
        laporan.append("-".repeat(77)).append("\n");
...
public class app {
    ...
    static LaporanKeuangan laporanKeuangan;

    public static void main(String[] args) {
        initpilihanCucian();
        laporanKeuangan = new LaporanKeuangan(daftarLayanan);
            ...
            while (running) {
                ...
                try {
                    ...
                    switch (pilih) {
                        ...
                        case 3 -> manajemenLaporanKeuangan();
                        case 4 -> {
                            running = false;
                            loginAdmin.logout();
                        }
    ...
    static void manajemenLaporanKeuangan() {
        System.out.println("\n=== Manajemen Laporan Keuangan ===");
        System.out.println("1. Lihat Total Pendapatan");
        System.out.println("2. Buat Laporan Transaksi");
        ...
        try {
            ...
            switch (pilihan) {
                case 1 -> {
                    System.out.println("\n=== Total Pendapatan ===");
                    System.out.printf("Total Pendapatan dari Layanan: Rp%.0f\n", laporanKeuangan.hitungTotalPendapatan());
                }
                case 2 -> {
                    System.out.print("Masukkan periode laporan (contoh: May 2025): ");
                    String periode = scanner.nextLine();
                    System.out.println("\n" + laporanKeuangan.buatLaporanTransaksi(periode));
                }
                case 3 -> { }
    ...
    static final void lihatDetailCucian() {
        ...
        try {
            ...
            if (pilihan >= 1 && pilihan <= daftarCucian.size()) {
                ...
                System.out.println("Pilihan ada, silahkan coba lagi");
            }
        } catch (NumberFormatException e) {
            System.out.println("Masukkan angka yang valid");
        }
    ...
    static void tambahLayanan() {
        try {
            ...
            if (jenisSepatu.trim().isEmpty()) {
                System.out.println("Jenis sepatu tidak boleh kosong");
                return;
            }
            
            System.out.print("Masukkan jumlah sepatu: ");
            int jumlahSepatu = Integer.parseInt(scanner.nextLine());
            if (jumlahSepatu <= 0) {
            ...
            if (pilihan >= 1 && pilihan <= 3) {
                ...
                System.out.println("Pilihan jenis cucian tidak valid");
            }
        } catch (NumberFormatException e) {
            System.out.println("Masukkan jumlah sepatu dan jenis cucian harus berupa angka");
        }
    }
    ...
    static void updateStatusLayanan() {
        ...
        try {
            int noPesanan = Integer.parseInt(scanner.nextLine());
            ...
            for (Layanan layanan : daftarLayanan) {
                if (layanan.noPesanan == noPesanan) {
                    ...
                    if (status.equalsIgnoreCase("Proses") || status.equalsIgnoreCase("Selesai")) {
                        layanan.status = status;
                        System.out.println("Status berhasil diperbarui!");
                        found = true;
                        break;
                    } else {
                        System.out.println("Error: Status hanya boleh Proses atau Selesai!");
                    }
                }
            }
            
            if (!found) {
                System.out.println("Error: Pesanan dengan nomor " + noPesanan + " tidak ditemukan!");
            }
        } catch (NumberFormatException e) {
            System.out.println("Error: Nomor pesanan harus berupa angka!");
        }
    }
    ...
    static void hapusLayanan(){
        ...
        try {
            ...
            int initialSize = daftarLayanan.size();
            daftarLayanan.removeIf(layanan -> layanan.noPesanan == noPesanan);
            
            if (daftarLayanan.size() < initialSize) {
                System.out.println("Layanan berhasil dihapus!");
            } else {
                System.out.println("Pesanan dengan nomor " + noPesanan + " tidak ditemukan");
            }
        } catch (NumberFormatException e) {
            System.out.println("Nomor pesanan harus berupa angka");
        }
    }
